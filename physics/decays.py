...
def _two_body_decay(parent_mass, m1, m2, rng: Optional[np.random.Generator] = None):
    ...
    px, py, pz = (
        p * sintheta * math.cos(phi),
        p * sintheta * math.sin(phi),
        p * costheta,
    )
...
def _three_body_decay(
    parent_mass, m1, m2, m3, rng: Optional[np.random.Generator] = None
):
    """
    ...
    p = math.sqrt(max(0, (m12**2 - (m1 + m2) ** 2) * (m12**2 - (m1 - m2) ** 2))) / (
        2 * m12
    )
...
def decay_particle(
    ...
    if len(daughter_masses) == 2:
        ...
        return _three_body_decay(
            parent_mass, daughter_masses[0], daughter_masses[1], daughter_masses[2], rng
        )
    else:
